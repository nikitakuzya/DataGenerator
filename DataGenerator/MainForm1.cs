    ...
    public partial class MainForm : Form
    {
        ...
        private void ReadOip()
        {
                ...
                //c 9-й версии добавился столбец OIPType, добавляем его следующим блоком.
            {
                ...
                blocks.Add(
                    sheetOip.Range[
                        sheetOip.Cells[startRow, 46],
                        sheetOip.Cells[endRow, 46]].Value); //7 - иерархия
            ...
            for (int i = 0; i < range.GetLength(0); i++)
            {
                try
                {
                    ...
                    if (Program.Settings.DB_version > 8)
                    {
                        if (range[i, 24] != null) rowObjects[30] = stationTag + range[i, 24]; //OPCPATH
                ...
                {
                    toolStripStatusLabel1.Text = toolStripStatusLabel1.Text + " Ошибка oip №" + i + " ";
                    continue;
                }
                    ...
                    case "diagDropSARD":
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i < rangeDropSARD.GetLength(0) - 1; i++)
                            {
                                ...
                                rowObjects[3] = rangeDropSARD[i + 1, 1].ToString() + " " + rangeDropSARD[i + 1, 7].ToString();
                                rowObjects[4] = -1;
                                rowObjects[5] = -1;
                                rowObjects[6] = -1;
                                if (Program.Settings.DB_version > 6)
                            ...
                            for (int i = 0; i < rangeDropSARD.GetLength(0) - 1; i++)
                            {
                                if (rangeDropSARD[i + 1, 5] != null && rangeDropSARD[i + 1, 5].ToString().Trim() != "")
                                {
                                    for (int j = 8; j < rangeDropSARD.GetLength(1); j++)
                                    {
                                        ...
                                        rowObjects[3] = rangeDropSARD[i + 1, 1].ToString() + " " + rangeDropSARD[i + 1, j].ToString() + " (" + rangeDropSARD[i + 2, j].ToString() + ")";
                                        rowObjects[4] = -1;
                                        rowObjects[5] = -1;
                                        rowObjects[6] = -1;
                                        if (Program.Settings.DB_version > 6)
                            ...
                            for (int i = 0; i < rangeDropSARD.GetLength(0) - 1; i++)
                            {
                                ...
                                if (str.ToUpper().Contains("КЦ") || str.ToUpper().Contains("КОНТРОЛЛЕР")) //пропускаем если не плк
                                {
                                    ...
                                    for (int j = 8; j < rangeDropSARD.GetLength(1); j++)
                                    {
                                        ...
                                        rowObjects[1] = row[1]; //sysId
                                        rowObjects[2] = 16 * (numPlc - 1) + (j - 7);
                                        rowObjects[3] = rangeDropSARD[i + 1, 4].ToString() + " " + rangeDropSARD[i + 1, j].ToString() + " (" + rangeDropSARD[i + 2, j].ToString() + ")";
                                        rowObjects[4] = -1;
                                        rowObjects[5] = -1;
                                        rowObjects[6] = -1;
                                        if (Program.Settings.DB_version > 6)
        ...
        private void ReadTimer() //чтение = генерация из Excel
        {
            ...
            for (int index = 0; index < listTimer.Length; index++)
            {
                ...
                int endRow = _fileIo.LastRowCell(ref sheetSys);
                ...
                switch (listTimerName)  //Рез;twList4;twNA;twCommTimes;twZDV;twVS
                {
                    ...
                    case "Защ НА":
                        ...
                        for (int i = 1; i <= rangeTable.GetLength(0); i++)
                        {
                            ...
                            if (rangeTable[i, 14] == null || rangeTable[i, 14].ToString() != "1")
                            {
                                continue; //Проверка на наличие защиты
                            }
                    ...
                    case "twNA": //+prot
                        ...
                        int endRowTwNA = Program.Settings.Quntity_twCommTimes + 1;
                       rangeTable = sheetSys.Range[sheetSys.Cells[startRow-1, 1], sheetSys.Cells[endRowTwNA, 20]].Value;
